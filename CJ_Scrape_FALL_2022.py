...
def pagePush(driver, wb, s1, row):
    ...
    print('Applicants: ' + str(len(apps)))
    print(apps[1].get_attribute('href'))
    print(apps[2].get_attribute('href'))
    #Loop through apps
    pg = 0
    while(pg < len(apps)):
        #Open applicant page
        print('READING: ' + str(pg))
        print('URL: ' + apps[pg].get_attribute('href'))
        driver.get(apps[pg].get_attribute('href'))
        time.sleep(delay())
